...
#        print(f"thinkdeb6 batch /mean.shape {batch.shape}   {means.shape}") 
...
#clt    used_subsample_len = (65 // args.step_length) * args.step_length
    ...
    for init_batch, target_batch, _ in tqdm(loader_standard, disable=rank != 0):
        ...
        for ss_i in range(args.step_length):
            ...
            slice_shape = batch[:, ss_i : used_subsample_len : args.step_length].shape
    ...
    if distributed and world_size > 1:
        ...
        if rank == 0:
            ...
            print(f"thinkxxx2 diff_means_gathered shape before cat", [t.shape for t in diff_means_gathered])
            ...
            print(f"thinkxxx2 diff_means_gathered shape after cat", diff_means_gathered.shape)
            ...
            print(f"thinkxxx2 original_indices", len(original_indices), original_indices[:10])
